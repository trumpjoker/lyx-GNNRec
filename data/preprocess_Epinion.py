...
The original Extended_epinion data can be found at:
http://www.trustlet.org/epinions.html
'''

# parser = argparse.ArgumentParser()
# parser.add_argument('--dataset', default='sample', help='dataset name: Douban/Epinion')
# opt = parser.parse_args()
# print("SR-GNN opt is:",opt)
#
...
# elif opt.dataset == 'Epinion':
#     PATH_TO_DATA = 'data/Extended_epinion/'
#     SOCIAL_NETWORK_FILE = PATH_TO_DATA + 'user_contacts.txt'
#     RATING_FILE = PATH_TO_DATA + 'rating.txt'

PATH_TO_DATA = 'data/Extended_epinion/'

SOCIAL_NETWORK_FILE = PATH_TO_DATA + 'user_contacts.csv'
RATING_FILE = PATH_TO_DATA + 'rating.csv'
max_length = 30
...
def process_rating(day=7):  # segment session in every $day days.
    ...
    df = pd.read_csv(RATING_FILE, sep='\t',
                     names=['ItemId', 'UserId', 'Rating', 'Status', 'Date', 'L_Time', 'Type', 'VerticalId'])
    ## loc
    df['Timestamp'] = df['Date'].apply(lambda x: time.mktime(time.strptime(x, '%Y/%m/%d')))
    df = df.loc[:, ['UserId', 'ItemId', 'Rating', 'Timestamp']]
    # df = df[:10]
    ...
    span_left = 9.7e8  ## 2001/01/10   979056000
    span_right = 1.08e9  ## 2003/8/12   1060617600
    ...
    print("min time is :", min_timestamp)
    print("max time is:", max_timestamp)
...
def process_social():  # read in social network.
    ## 读取socialnet.tsv文件
    net = pd.read_csv(SOCIAL_NETWORK_FILE, sep='\t', names=['Follower', 'Followee', 'Value', 'Date'])
    ## 删除特定的列['Follower', 'Followee']的重复行
    net.drop_duplicates(subset=['Follower', 'Followee'], inplace=True)
    net = net.loc[net['Value'].isin([1])]
    # net = net[:10]
    net = net.loc[:, ['Follower', 'Followee', 'Value']]
...
def split_data(day):  # split data for training/validation/testing.
    ...
    # 以最大TimeID的周前作为基准划分训练集，训练集是由Session_ID组成
    ...
    session_train = session_max_times[session_max_times < tmax * 0.8].index
    session_holdout = session_max_times[session_max_times >= tmax * 0.8].index
    print("sess train:", len(session_train))
    print("sess holdout:", len(session_holdout))
    ...
    # 筛选项目出现次数大于等于15的数据和会话长度大于1的数据
    train_tr = train_tr[train_tr['ItemId'].groupby(train_tr['ItemId']).transform('size') >= 15]
    ...
    ## 选择训练集中出现次数大于等于20的ItemId
    to_predict = Counter(el for el in train_item_counter.elements() if train_item_counter[el] >= 15).keys()  # 7531
    ...
    ## 筛选会话长度大于1的会话
    valid = valid[valid['UserId'].isin(train_tr['UserId'].unique())]
    ...
    ## 筛选会话长度大于1的会话
    test = test[test['UserId'].isin(train_tr['UserId'].unique())]
    ...
    with open('data/user_id_map.tsv', 'w') as fout:
        for k, v in user_map.items():
            fout.write(str(k) + '\t' + str(v) + '\n')
    with open('data/item_id_map.tsv', 'w') as fout:
    ...
    valid = valid.sort_values(by=['UserId', 'TimeId'], ascending=True)
    test = test.sort_values(by=['UserId', 'TimeId'], ascending=True)
